...
async function bootstrap(): Promise<void> {
    ...
    for (let page = 1; page <= totalPages; page++) {
        ...
        while (true) {
            try {
                ...
                attempt++;
                const status429 = err?.response?.status === 429 || String(err).includes('429');
                if (status429 && attempt <= maxRetries) {
                    const backoff = baseDelay * Math.pow(2, attempt);
                    console.warn(`⚠️ 429 reçu pour page ${page}, retry #${attempt} après ${backoff}ms`);
                    await sleep(backoff);
                    continue;
                }
    ...
    console.log(`🎉 Scraping terminé : ${muIds.length} muId(s) écrits dans ${filePath}`);
