    ...
    for (const muId of muIds) {
        ...
        while (attempt <= maxRetries) {
            try {
                ...
                if (status === 429 && attempt <= maxRetries) {
                    ...
                    console.warn(`⚠️ 429 pour ${muId}, retry #${attempt} après ${backoff}ms`);
                    await sleep(backoff);
                    continue;
                }
                console.error(`❌ Échec ${muId} (tentative ${attempt}/${maxRetries}) :`, err.message || err);
                break;
            }
        }
        ...
        if (!success) {
            ...
            if (consecutiveFails >= maxConsecutiveFails) {
                console.error(`🚨 ${consecutiveFails} échecs consécutifs atteints. Arrêt du script.`);
                process.exit(1);
            }
        }
